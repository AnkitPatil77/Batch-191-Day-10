...
//		companies = new CompanyEmpWage[10];
	...
	public void addCompany(String companyName, int fullTimeWorkingHour, int wagePerHour, int maxWorkingDay,
			int maxWorkingHour) {
		CompanyEmpWage companyEmpWage = new CompanyEmpWage(companyName, fullTimeWorkingHour, wagePerHour, maxWorkingDay,
				maxWorkingHour);
...
//			}
	...
	public static void main(String[] args) {
		...
		System.out.println("ICICI's emp salary : $" + testObj.getTotalSalary("icici") + " USD");
	}

	public double getTotalSalary(String companyName) {
		for (int i = 0; i < companiesEmps.size(); i++) {
			if (companiesEmps.get(i).getCompanyName().toLowerCase().equals(companyName.toLowerCase())) {
